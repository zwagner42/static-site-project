...
def generate_pages_recursive(dir_path_content: Path, template_path: Path, dest_dir_path: Path):
    ...
    for content in dir_path_content.iterdir():
        if content.is_file() and content.suffix == ".md":
            generate_page(dir_path_content / content.name, template_path, dest_dir_path / (content.stem + ".html"))
        elif content.is_dir():
            generate_pages_recursive(dir_path_content / content.name, template_path, dest_dir_path / content.name)
